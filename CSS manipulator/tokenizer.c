    ...
    if (fp == NULL) {
        ...
        while ((ch = fgetc(fp)) != EOF) {
            ...
            if (DYN_IsTokenBoundary(ch)) {                //if it is a token boundary
                if (tempStrIndex >
                    0) {                    //and we have a string, signalled by tempStrIndex's walk on the number line towards right
                ...
                if (space_seen <=
                    1) {                        //space_seen==0 means inside a token already, 1 means first char of token.
